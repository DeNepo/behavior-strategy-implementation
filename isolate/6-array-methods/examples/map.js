...
// write isEnthusiastic to pass the tests
...
const _1_array = [];
const _1_expect = [];
const _1_test = deepCompare(_1_array.map(truthiness), _1_expect);
console.assert(_1_test, 'Test 1');

// works when all values are truthy
const _2_array = [100, 'hello', true];
const _2_expect = ['truey', 'truey', 'truey'];
const _2_test = deepCompare(_2_array.map(truthiness), _2_expect);
console.assert(_2_test, 'Test 2');

// works when all values are falsy
const _3_array = ['', 0, NaN, null];
const _3_expect = ['falsey', 'falsey', 'falsey', 'falsey'];
const _3_test = deepCompare(_3_array.map(truthiness), _3_expect);
console.assert(_3_test, 'Test 3');

// works when there are mixed values
const _4_array = [true, 0, NaN, 'hello'];
const _4_expect = ['truey', 'falsey', 'falsey', 'truey'];
const _4_test = deepCompare(_4_array.map(truthiness), _4_expect);
